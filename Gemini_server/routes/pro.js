...
router.post("/login",(req,res)=>{
	...
	//查询数据库
	var sql="select * from gmn_user where "
	+"uname=? and upwd=?";
...
router.get("/deleteUser",(req,res)=>{
	...
	var sql="delete from gmn_user where uid=?";
...
router.get("/query",(req,res)=>{
	...
	//查数据库 
	var sql="select * from gmn_user where uid=?";
	pool.query(sql,[$uid],(err,result)=>{
		if(err) throw err;
		if(result.length>0){
			res.send(result[0]);
...
router.post("/update",(req,res)=>{
	...
	//插入数据库
	var sql="update gmn_user set uname=?,"
